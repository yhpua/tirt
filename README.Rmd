...
Suppose the `ffi_walk` item is highly similar or collinear with the anchor condition. By specifying `clvar = "ffi_walk"`, `tirt` will remove this item in the initial estimation phase. Once `tirt` has estimated the FI threshold in terms of the latent trait level, it will re-fit an IRT model on all FI items (and excluding the anchor item), and use these IRT parameters to find the expected FI score that corresponds to the latent threshold.   
